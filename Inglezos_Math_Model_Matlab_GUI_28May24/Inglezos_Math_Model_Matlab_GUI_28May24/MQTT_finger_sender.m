classdef MQTT_finger_sender < handle
    ...
    methods
        ...
        function addToDataBatch(obj, t, endEffectorPosition, jointAngles)
            % Add data to the data batch

            % Validate inputs
            if ~isnumeric(t) || ~isnumeric(endEffectorPosition) || ~isnumeric(jointAngles)
                error('MATLAB:invalidType', 'Input arguments must be numeric.');
            end
        ...
        function sendDataBatch(obj)
            % Send the entire data batch to the MQTT broker
