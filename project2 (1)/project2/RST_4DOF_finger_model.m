classdef RST_4DOF_finger_model < handle
    ...
    methods
        ...
        function setupIKSolver(obj, iks_max_iter)
            ...
            obj.ikSolver = inverseKinematics('RigidBodyTree', obj.fingerRobotModel);
        ...
        function moveFingerToPosition(obj, position, show)
            % Validate position
            if ~obj.isValidPosition(position)
                error('MATLAB:invalidPosition', 'The specified position is invalid.');
            end

            % Move the finger to the specified position
        ...
        function valid = isValidPosition(~, position)
            % Define criteria for a valid position
            % Adjust the range according to your specific requirements
            valid = all(position >= 0 & position <= 1);
        end

        function finger_ee_pos = getFingerEEPosition(obj)
            % Get the finger end-effector position
